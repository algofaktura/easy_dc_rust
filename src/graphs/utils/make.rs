...
pub fn make_vi_mapping(verts: &Vectors3d) -> VertIdx {
    verts.iter()
         .enumerate()
         .map(|(idx, vert)| (vert, idx as u32)).collect::<VertIdx>()
}

pub fn make_edges_adj(a: &Adjacency, edges: &Edges) -> EdgeAdjacency {
    edges.iter().map(|&(u, p)| {
        let lhs = a.get(&u).unwrap().difference(&HashSet::from([p])).cloned().collect::<Neighbors>();
        let rhs = a.get(&p).unwrap().difference(&HashSet::from([u])).cloned().collect::<Neighbors>();
        let prod = lhs.iter().flat_map(|&m| rhs.iter().map(move |&n| if m < n { (m, n) } else { (n, m) })).collect::<Edges>();
