...
use crate::{
    graph::{types::{Adjacency, Edge, EdgeAdjacency, Edges, Solution, Thread, Tour, VertsC3i16}, check::is_valid_edge_i16},
};
...
impl<'a> Cycle<'a> {
    ...
    pub fn edges(&mut self) -> Edges {
        if self.prev != self.data {
            ...
            .into_iter()
            .map(|(a, b)| if a < b { (a, b) } else { (b, a) })
            .filter(|&(a, b)| is_valid_edge_i16(self.verts[a as usize], self.verts[b as usize]))
            .collect();
