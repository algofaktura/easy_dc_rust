...
use crate::{
    ...
    types::types::{
        Adjacency, Bobbins, Loom, Spool, Thread, Tour, Vectors3d, VertIdx, VertsC3, Warps, Woven,
        Yarn, Point, Count,
    },
};
...
pub fn get_warps(zlevel: Point, order: Count, bobbins: &Bobbins, spool: &Spool, vert_idx: &VertIdx) -> Warps {
            ...
            if !woven.contains(&idx) {
                match (thread.front(), thread.back()) {
                    (Some(f), _) if *f == warp[0] => {
                        *thread = warp[1..].iter().rev().chain(thread.iter()).cloned().collect()
                    }
                    (_, Some(b)) if *b == warp[0] => {
                        thread.extend(warp.iter().skip(1))
                    }
                    _ => { continue; }
                }
                woven.extend([idx])
