...
pub fn find_solution(level: u32, _certify: bool) -> Result<Solution, &'static str> {
    ...
    // Solve graph.
    start = Instant::now();
    let result = weave::weave(n as usize, z_adj, z_order, min_xyz, order);
    let dur_solve = Instant::now() - start;
    println!("🇳 {n:>4} FINISHED WEAVING. 🔎 CERTIFYING SOLUTION...");
    // Certify Solution.
    start = Instant::now();
    let seq_id = is_hamiltonian_circuit(&result, order as usize, min_xyz + 8);
    let dur_certify = Instant::now() - start;
    // Print out results and times.
    println!(
    ...
    // Test that solution is a HamCycle
