...
pub fn solve_node_version(order: u32, repeats: u32) {
    ...
    let max_xyz16 = get_max_xyz_i16(order as i32);
    let verts: Verts = make_vertices(max_xyz);
    let verts16 = make_vertices_i16(max_xyz as i16);
    let v3verts: Vectors3d = from_v3c_to_vect3d(&verts);
    let vert_idx: HashMap<(i32, i32, i32), u32> = make_vi_mapping3(&verts);
    println!("MAX XYZ i32 {:?} i16 {:?} | V {:?}, V16 {:?} {:?}", max_xyz, max_xyz16, verts, verts16, verts.len());
    ...
    println!("⭕️ ORDER: {:?} | ID: {} | LEN: {:?} | DUR: {:?}", order, id, solution.len(), dur);
}