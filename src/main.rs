...
use graphs::data::g_448::{ADJ, EDGES, VAR, VERTS};
use graphs::info::certify::{id_seq, SequenceID, SequenceID::HamCycle};
use graphs::utils::make::{make_edges_adj, make_vi_mapping, make_weights};
use graphs::utils::map::{convert_from_nodes, map_graph, vectorize};
...
fn weave(
    ...
    let loom: WarpedLoom = wefts
        .iter()
        .enumerate()
        .map(|(idx, seq)| (idx, Cycle::new(&seq, &adj, &edge_adj, VERTS, false)))
        .collect();
    let mut done: Done = HashSet::new();
    if loom.keys().len() > 0 {
        'weaving: loop {
            for idx in loom.keys() {
                if done.len() == loom.keys().len() {
                    break 'weaving;
                }
                if done.len() == loom.keys().len() - 1 {
                    warp.set_last()
                }
...
fn warp_loom(v3verts: &Vectors3d, adj: &Adjacency, vert_idx: &VertIdx) -> Loom {
    ...
    for (zlevel, order) in z_length {
                    ...
                    for end in vec![0 as usize, thread.len() - 1] {
                        if thread[end] == warp[0 as usize] {
                            ...
                            if end == 0 as usize {
                                warp[1..]
                                    .iter()
                                    .map(|item| thread.push_front(*item))
                                    .fold((), |_, _| ());
                            } else {
        ...
        for (_, seq) in warps.iter().enumerate().filter(|(i, _)| !woven.contains(i)) {
            loom.extend(vec![VecDeque::from(
                seq.iter().cloned().collect::<Thread>(),
            )]);
        }
        if zlevel != -1 {
