...
use crate::data::g_32::{VERTS, ADJ, EDGES};
use crate::graphs::reduce::shrink_adjacency;
use crate::graphs::make::{make_weights, make_vi_mapping, make_edges_adj};
...
fn weave() -> Vec<u32> {
    ...
    let mut warp_wefts = warp_loom(&adj);
    println!("{:?}", warp_wefts);
    ...
    let loom: HashMap<usize, &mut Cycle> = wefts
        .iter()
        .enumerate()
        .map(|(idx, seq)| (idx, Cycle::new(&seq, &adj, &edge_adj)))
        .collect();
    let last_idx: usize = loom.len() - 1;
    let mut processed: HashSet<usize> = HashSet::new();
    'weaving: loop {
        for idx in loom.keys() {
            if processed.len() == loom.keys().len() { break 'weaving }
            if processed.contains(idx) { continue }
            if *idx == last_idx { warp.set_last() }
            ...
            if !bridge.is_empty() {
                ...
                let mut weft_es = edge_adj.get(&warp_e).unwrap().intersection(&other.edges())
                    .into_iter()
                    .cloned()
...
fn warp_loom(adj: &HashMap<u32, HashSet<u32>>) -> Vec<VecDeque<u32>> {
