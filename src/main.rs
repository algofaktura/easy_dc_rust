...
fn main() {
    ...
    use graphs::data::g_1320::{ADJ, EDGES, VAR, VERTS};
    let args: Vec<String> = env::args().collect();
    let order: u32 = args.get(1).unwrap_or(&"1320".to_string()).parse().unwrap();
    let repeats: u32 = args.get(2).unwrap_or(&"10000".to_string()).parse().unwrap();
