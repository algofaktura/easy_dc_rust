...
use graph::types::{Adjacency, Verts, Edges, Solution};
...
pub fn solve_node_version(order: u32, repeats: u32) {
    ...
    for _ in 0..repeats - 1 {
        weave(&adj, &vert_idx, &edge_adj, &verts, &var, &weights, &z_adj, &z_length);
    }
    let solution: Solution = weave(&adj, &vert_idx, &edge_adj, &verts, &var, &weights, &z_adj, &z_length);
